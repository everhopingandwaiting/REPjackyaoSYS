...
public class UpdatePurchaseItemEditDialog extends JDialog
{
 ...
 private    PurchaseItem purchaseItem=new PurchaseItem();
  ...
  public UpdatePurchaseItemEditDialog(PurchaseItem purchaseItem,int selectTableRow) {
	 
  super(MainFrame.getInstance(), "编辑采购明细", true);
	 // setTitle("编辑采购明细");
    	this.purchaseItem=purchaseItem;
    	this.selectTableRow=selectTableRow;
    	System.out.println(this.purchaseItem);
    setSize(320, 240);
    setResizable(false);
  ...
  public void createContentPanel(JPanel cp) {
    ...
    addComponentListener(new ComponentAdapter()
    {
      public void componentShown(ComponentEvent e) {
        List list = UpdatePurchaseItemEditDialog.this.prodDao.findAll();
       prod_model = new DefaultComboBoxModel(new Vector(list));
        ...
        UpdatePurchaseItemEditDialog.this.cbx_prod.addItemListener(new ItemListener()
        {
          public void itemStateChanged(ItemEvent e) {
            if (e.getStateChange() == 1) {
              Product prod = (Product)UpdatePurchaseItemEditDialog.this.cbx_prod.getSelectedItem();
              UpdatePurchaseItemEditDialog.this.tfd_price.setText(UpdatePurchaseItemEditDialog.this.nf.format(prod.getSale_price().doubleValue()));
            }
          }
        });
      }
  ...
  public void createBtnPanel(JPanel cp)
  {
    ...
    jbt2.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        if (UpdatePurchaseItemEditDialog.this.cbx_prod.getItemCount() >= 0) {
          UpdatePurchaseItemEditDialog.this.cbx_prod.setSelectedIndex(0);
        }
        UpdatePurchaseItemEditDialog.this.tfd_num.setText("1");
      }
    });
    btnPanel.add(jbt2);
